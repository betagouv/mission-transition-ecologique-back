...
- `src/shared/` — helpers réutilisés par tous les formats : `ThemeMapper`, `TypeAideMapper`, `NafSectionResolver`, `RegionNameResolver`, `ExportPolicy.isPublished`, `StatutMapper` (collapse `statut_dispositif` → `actif`/`indisponible`, **export Grist/open data uniquement** ; AGIR a son propre vocabulaire via `AgirEtatMapper`), `ExportLogger`/`ConsoleExportLogger`.
- `src/tee/` — round-trip `programs.json` : `TeeImporter` (`programs.json` → pivot), `TeeExporter` (pivot → `programs.json`, self-check aller-retour), `SlugCanonicalId` (cuid2 déterministe dérivé du slug, pour l'import sans Payload), `ProgramRedirects` (lit `program_redirects` de `redirects.json` amont : ancien slug → slug courant) + `RedirectTombstoneBuilder` (synthétise des dispositifs `remplace` en clonant le contenu de la cible sous l'ancien slug, `remplace_par` = id de la cible ; les redirections dont la cible est absente sont ignorées et journalisées). Ces tombstones alimentent AGIR (statut `remplace`) mais restent hors export Grist (`SchemaExportPolicy` filtre `remplace`).
- `src/agir/` — **triple export AGIR**. `AgirVocabulary` (chaînes AGIR centralisées), mappers `AgirSourceMapper`/`AgirEtatMapper` (statut unique `en_prod`/`temporairement_indisponible`/`remplace`, `archive`→`en_prod`)/`AgirThemeMapper` (thèmes wire à 7 valeurs, famille environnement repliée)/`AgirContactMapper` (`ADEME`→`ademe`)/`AgirTypeDispositifMapper`, `RemplaceParResolver` (cuid2 `remplace_par` → slug du remplaçant), filtre `AgirExportPolicy` (publié + statut exportable, inclut `remplace`/`archive` ; l'archivage est transmis, porté par `date_cloture`), exporters `AgirListeExporter` (index + 2 URLs, base URL injectée, `idDispositif` = slug), `AgirDetailExporter` (`DetailDispositif`, proposition 1 R2DA), `AdemePivotExporter` (`AdemePivot`, proposition 2 = canonical wire + deltas ADEME, liste blanche `.strict()`, `remplace_par` résolu injecté via `RemplaceParResolver`). Types + garde-fous zod `agir-detail.schema.ts`/`ademe-pivot.schema.ts`. Voir `docs/context/agir-export-format.md`.
- `src/schema/` — **export schéma Etalab → Grist** (`dispositif-aide` + extension `professionnels`). `SchemaVocabulary` (libellés/défauts centralisés), `SchemaIdResolver` (UUID v5 du slug, sans dépendance), mappers `TypesAidesSchemaMapper`/`GeoCoverageMapper`/`SecteurActiviteMapper`/`PorteursMapper` + builders texte `DescriptionTextBuilder`/`EligibiliteTextBuilder` (replient montant/durée/effectif dans les colonnes texte du core), `SchemaProgramMapper` (→ `SchemaRow`), `SchemaFitChecker` (schémas satisfaits), `SchemaExportPolicy` (publié + exportable, réutilise les mappers AGIR), `GristRowBuilder` (colonnes + `slug` + colonne `technical` JSON = métadonnées + canonical brut), `GristExporter` (orchestrateur `exportMany`), `EtalabSchemaValidator` (validation contre les Table Schema embarqués dans `src/schema/etalab/`). Voir `docs/context/schema-grist-export.md`.
- `src/grist/` — client REST : `GristConfig` (env `GRIST_BASE_URL`/`GRIST_DOC_ID`/`GRIST_TABLE_ID`/`GRIST_API_KEY`), `GristClient.upsertMany` (PUT, clé d'upsert = `slug`, `fetch` injectable), `gristColumns.ts` (`GRIST_COLUMNS` = source de vérité des colonnes, `id` Etalab porté sous `rnasp_id`), `GristTableManager` (amorçage REST : crée la table + aligne les colonnes, idempotent). Ni l'upsert des lignes ni le widget data.gouv ne créent la table : `setup:grist` doit l'amorcer en premier.
- `src/__fixtures__/canonical-programs.ts` — golden fixtures (`minimal`/`full` + `draft`/`indisponible`/`archived` pour les filtres).
- Targets `export:tee`, `import:tee`, `export:grist` et `setup:grist` (`scripts/*.ts`). `import:tee` régénère le store canonical depuis `static/input/programs.json` **sans Payload** (`TeeImporter` + `SlugCanonicalId`), puis applique `static/input/redirects.json` (`ProgramRedirects` + `RedirectTombstoneBuilder`) pour transmettre les redirections en dispositifs `remplace`. `setup:grist` amorce la table Grist (crée + aligne les colonnes, `-- --prune` pour purger les colonnes par défaut). `export:grist` est un **dry run** par défaut (snapshot `static/exports/grist-records.json` + récap) ; `--push` (ou `GRIST_PUSH=1`) requis pour écrire dans Grist.
- **Pipeline quotidien** : `.github/workflows/daily_data.yml` (cron + `workflow_dispatch`) récupère `programs.json` + `redirects.json` amont → `import:tee` (store frais + tombstones de redirection) → `setup:grist` → `export:grist` (push) → commit `programs.json` + `redirects.json` + `canonical.db`. Secrets requis : `GRIST_BASE_URL`/`GRIST_DOC_ID`/`GRIST_TABLE_ID`/`GRIST_API_KEY`. Voir `docs/context/schema-grist-export.md`.
